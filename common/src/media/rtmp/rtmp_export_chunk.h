...
class RtmpExportChunk : public RtmpMuxUtil {
 ...
 public:
  std::shared_ptr<std::vector<uint8_t>> ExportStreamData(std::shared_ptr<RtmpMuxMessageHeader> &message_header,
                                                         std::shared_ptr<std::vector<uint8_t>> &data);

 private:
  void Destroy();
  std::shared_ptr<ExportStream> GetStream(uint32_t chunk_stream_id);
  std::shared_ptr<RtmpChunkHeader> GetChunkHeader(std::shared_ptr<ExportStream> &stream,
                                                  const std::shared_ptr<RtmpMuxMessageHeader> &message_header);
