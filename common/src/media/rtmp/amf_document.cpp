...
int AmfUtil::DecodeNumber(const void *data, double *number) {
  ...
  // marker 체크
  if (ReadInt8(pt_in) != (int)AmfTypeMarker::Number) {
    return 0;
  }
  pt_in++;
...
int AmfUtil::DecodeBoolean(const void *data, bool *boolean) {
  ...
  // marker 체크
  if (ReadInt8(pt_in) != (int)AmfTypeMarker::Boolean) {
    return 0;
  }
  pt_in++;
...
int AmfUtil::DecodeString(const void *data, char *string) {
  ...
  // marker 체크
  if (ReadInt8(pt_in) != (int)AmfTypeMarker::String) {
    return 0;
  }
  pt_in++;
...
int AmfObjectArray::Decode(const void *data, int data_length) {
  ...
  // 시작 marker 확인
  if (ReadInt8(pt_in) != start_marker) {
    return 0;
  }
  pt_in++;
...
//====================================================================================================
int AmfDocument::GetPropertyCount() { return (int)_amf_propertys.size(); }
...
int AmfDocument::GetPropertyIndex(char *name) {
  ...
  // 찾기
  for (i = 0; i < (int)_amf_propertys.size(); i++) {
    if (_amf_propertys[i]->GetType() != AmfDataType::String) {
      continue;
    }
    if (strcmp(_amf_propertys[i]->GetString(), name)) {
      continue;
    }
