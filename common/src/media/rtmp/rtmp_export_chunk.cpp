...
// 소멸자
//====================================================================================================
RtmpExportChunk::~RtmpExportChunk() { Destroy(); }

//====================================================================================================
// 종료
//====================================================================================================
void RtmpExportChunk::Destroy() { _stream_map.clear(); }
...
//====================================================================================================
std::shared_ptr<RtmpChunkHeader> RtmpExportChunk::GetChunkHeader(
    std::shared_ptr<ExportStream> &stream, const std::shared_ptr<RtmpMuxMessageHeader> &message_header) {
  auto chunk_header = std::make_shared<RtmpChunkHeader>();

  // type 2
  if (_compress_header && stream->message_header->chunk_stream_id == message_header->chunk_stream_id &&
  ...
  // type 1
  else if (_compress_header && stream->message_header->chunk_stream_id == message_header->chunk_stream_id &&
           stream->message_header->stream_id == message_header->stream_id) {
...
// Export 스트림
//====================================================================================================
std::shared_ptr<std::vector<uint8_t>> RtmpExportChunk::ExportStreamData(
    std::shared_ptr<RtmpMuxMessageHeader> &message_header, std::shared_ptr<std::vector<uint8_t>> &data) {
  int buffer_size = 0;
  int export_size = 0;
  ...
  // 버퍼 크기 설정(최대치 계산)
  buffer_size = RTMP_PACKET_HEADER_SIZE_MAX + GetChunkDataRawSize(_chunk_size, message_header->chunk_stream_id,
                                                                  message_header->body_size, chunk_header->is_extended);
  ...
  export_size += GetChunkDataRaw(_chunk_size, message_header->chunk_stream_id, data, export_data->data() + export_size,
                                 chunk_header->is_extended, type3_time);
