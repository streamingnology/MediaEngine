...
#include "core/log.h"
off_t RtmpChunkParser::Parse(const std::map<uint32_t, std::shared_ptr<const RtmpChunkHeader>> &chunk_map,
...
RtmpChunkParser::ParseResult RtmpChunkParser::ParseMessageHeader(ov::ByteStream &stream, off_t *parsed_bytes) {
  ...
  switch (_current_chunk_header->basic_header.format_type) {
    case RtmpChunkType::T0:
      ...
      OV_ASSERT((stream.GetOffset() - current_offset) == _current_chunk_header->message_header_size,
                "Expected: %lld, But: %lld", _current_chunk_header->message_header_size,
                (stream.GetOffset() - current_offset));
...
RtmpChunkParser::ParseResult RtmpChunkParser::ParseExtendedTimestamp(std::shared_ptr<const RtmpChunkHeader> last_chunk,
                                                                     ov::ByteStream &stream, off_t *parsed_bytes) {
