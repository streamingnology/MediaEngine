...
struct RtmpMuxMessageHeader {
 public:
  RtmpMuxMessageHeader() : chunk_stream_id(0), timestamp(0), type_id(0), stream_id(0), body_size(0) {}

  RtmpMuxMessageHeader(uint32_t chunk_stream_id_, uint32_t timestamp_, uint8_t type_id_, uint32_t stream_id_,
                       uint32_t body_size_) {
...
class RtmpMuxUtil {
 ...
 public:
  ...
  static int GetChunkHeaderSize(RtmpChunkType chunk_type, uint32_t chunk_stream_id, int basic_header_size,
                                void *raw_data, int raw_data_size);  // ret:길이, ret<=0:실패
  static std::shared_ptr<RtmpChunkHeader> GetChunkHeader(void *raw_data, int raw_data_size, int &chunk_header_size,
                                                         bool &extend_type);  // ret<=0:실패, ret>0:처리길이
  static int GetChunkData(int chunk_size, void *raw_data, int raw_data_size, int chunk_data_size, void *chunk_data,
                          bool extend_type);  // ret=> 0:실패, 1:처리길이
  ...
  static int GetChunkDataRaw(int chunk_size, uint32_t chunk_stream_id,
                             std::shared_ptr<std::vector<uint8_t>> &chunk_data, void *raw_data, bool extend_type,
                             uint32_t time);
};
