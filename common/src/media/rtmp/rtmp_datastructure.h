...
struct RtmpChunkHeader {
  ...
  ov::String ToString() const {
    ...
    if (basic_header_size > 0U) {
      result.AppendFormat("<Size: %d, Type: %d, StreamID: %u, TS: %lld>, Message: ", basic_header_size,
                          static_cast<int>(basic_header.format_type) >> 6, basic_header.stream_id, completed.timestamp);

      if ((basic_header.format_type == RtmpChunkType::T3) || (message_header_size > 0U)) {
        switch (basic_header.format_type) {
          case RtmpChunkType::T0:
            result.AppendFormat("<Type0: TS: %u, Len: %u bytes, TypeID: %d, StreamID: %d>", header.type_0.timestamp,
                                header.type_0.length, header.type_0.type_id, header.type_0.stream_id);
            break;

          case RtmpChunkType::T1:
            result.AppendFormat("<Type1: dTS: %u, Len: %u bytes, TypeID: %d>", header.type_1.timestamp_delta,
                                header.type_1.length, header.type_1.type_id);
            break;

          case RtmpChunkType::T2:
            result.AppendFormat("<Type2: dTS: %u>", header.type_2.timestamp_delta);
            break;
