...
RtmpImportChunk::RtmpImportChunk(int chunk_size) : _vhost_app_name("") { _chunk_size = chunk_size; }

RtmpImportChunk::~RtmpImportChunk() { Destroy(); }
...
int64_t RtmpImportChunk::CalculateRolledTimestamp(int64_t last_timestamp, int64_t parsed_timestamp) {
  ...
  if (delta <= ((1LL << (SERIAL_BITS - 1)) - 1)) {
    ...
    logtd("Timestamp is rolled: last TS: %lld, parsed: %lld, new: %lld", last_timestamp, parsed_timestamp,
          new_timestamp);
  }
...
bool RtmpImportChunk::ProcessChunkHeader(const std::shared_ptr<RtmpChunkHeader> &chunk_header,
                                         const std::shared_ptr<const RtmpChunkHeader> &last_chunk_header) {
...
bool RtmpImportChunk::CalculateForType3Header(const std::shared_ptr<RtmpChunkHeader> &chunk_header) {
  ...
  int expected_type_3_size =
      chunk_header->basic_header_size + (chunk_header->is_extended ? sizeof(RtmpChunkHeader::extended_timestamp) : 0);
...
std::shared_ptr<const RtmpMessage> RtmpImportChunk::FinalizeMessage(
    const std::shared_ptr<const RtmpChunkHeader> &chunk_header, ov::ByteStream &stream) {
  ...
  while (payload_size > 0) {
    if (index > 0) {
      if (payload_size < static_cast<size_t>(basic_header_size + extended_header_size)) {
        logte("Not enough data: %zu bytes, expected: %zu bytes", payload_size,
              basic_header_size + extended_header_size);
        return nullptr;
      }
      ...
      if (::memcmp(current, expected_type_3_header, basic_header_size) != 0) {
        logte("Invalid message is received: offset: %lld\nexpected:\n%s\nbut:\n%s",
              ...
              ov::Dump(expected_type_3_header, basic_header_size).CStr(), ov::Dump(current, basic_header_size).CStr());
  ...
  if (payload_data->GetLength() != chunk_header->payload_size) {
    OV_ASSERT(payload_data->GetLength() != chunk_header->payload_size, "Payload length: %zu, Expected: %zu",
...
size_t RtmpImportChunk::GetMessageCount() const { return _message_queue.Size(); }

void RtmpImportChunk::SetAppName(const std::string &vhost_app_name) {
  _vhost_app_name = vhost_app_name;

  _message_queue.SetAlias(
      ov::String::FormatString("RTMP queue for %s/%s", _vhost_app_name.c_str(), _stream_name.CStr()));
}
