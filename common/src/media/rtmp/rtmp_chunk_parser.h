...
class RtmpChunkParser {
 public:
  ...
  bool IsParseCompleted() const noexcept { return (_parse_status == ParseStatus::Completed); }
 ...
 protected:
  ...
  enum class ParseResult { NeedMoreData, Completed, Failed };

  enum class ParseStatus { BasicHeader, MessageHeader, ExtendedTimestamp, Completed };
  ...
  ParseResult ParseExtendedTimestamp(std::shared_ptr<const RtmpChunkHeader> last_chunk, ov::ByteStream &stream,
                                     off_t *parsed_bytes);
