...
namespace cmn {
enum class MediaType : int8_t { Unknown = -1, Video = 0, Audio, Data, Subtitle, Attachment, Nb };
...
enum class MediaCodecId : uint8_t { None = 0, H264, H265, Vp8, Vp9, Flv, Aac, Mp3, Opus, Jpeg, Png };
...
class Timebase {
 public:
  ...
  Timebase(int32_t num, int32_t den) : _num(num), _den(den) {}
  ...
  void SetDen(int32_t den) { _den = den; }

  int32_t GetNum() const { return _num; }

  int32_t GetDen() const { return _den; }
  ...
  bool operator==(const Timebase &timebase) const { return (timebase._num == _num) && (timebase._den == _den); }

  bool operator!=(const Timebase &timebase) const { return operator==(timebase) == false; }

  ov::String GetStringExpr() const { return ov::String::FormatString("%d/%d", GetNum(), GetDen()); }

  ov::String ToString() const { return std::move(GetStringExpr()); }
...
class AudioSample {
 ...
 public:
  ...
  explicit AudioSample(Format fmt) { SetFormat(fmt); }
...
#define OV_MEDIA_TYPE_SET_VALUE(type, ...) \
  ...
  // Unit: Bytes
  int32_t GetSampleSize() const { return _sample_size; }

  AudioSample::Format GetFormat() const { return _format; }

  void SetRate(Rate rate) { _rate = rate; }

  Rate GetRate() const { return _rate; }

  int32_t GetRateNum() const { return static_cast<std::underlying_type<AudioSample::Rate>::type>(_rate); }
...
class AudioChannel {
 ...
 public:
  ...
  // channel layout
  AudioChannel::Layout GetLayout() const { return _layout; }

  // channel count
  uint32_t GetCounts() const { return _count; }
