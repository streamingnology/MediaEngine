...
class MediaPacket {
 public:
  ...
  MediaPacket(cmn::MediaType media_type, int32_t track_id, const std::shared_ptr<ov::Data> &data, int64_t pts,
              int64_t dts, cmn::BitstreamFormat bitstream_format, cmn::PacketType packet_type)
  ...
  MediaPacket(cmn::MediaType media_type, int32_t track_id, const std::shared_ptr<ov::Data> &data, int64_t pts,
              int64_t dts, int64_t duration, MediaPacketFlag flag, cmn::BitstreamFormat bitstream_format,
  ...
  // This constructor is usually used by the MediaRouter to send media packets to the publihsers.
  MediaPacket(cmn::MediaType media_type, int32_t track_id, const std::shared_ptr<ov::Data> &data, int64_t pts,
              int64_t dts, int64_t duration, MediaPacketFlag flag)
      : _media_type(media_type),
        _track_id(track_id),
        _data(data),
        _pts(pts),
  ...
  MediaPacket(cmn::MediaType media_type, int32_t track_id, const ov::Data *data, int64_t pts, int64_t dts,
              int64_t duration, MediaPacketFlag flag)
      : _media_type(media_type), _track_id(track_id), _pts(pts), _dts(dts), _duration(duration), _flag(flag) {
    if (data != nullptr) {
      _data = data->Clone();
    } else {
  ...
  MediaPacket(cmn::MediaType media_type, int32_t track_id, const void *data, int32_t data_size, int64_t pts,
              int64_t dts, int64_t duration, MediaPacketFlag flag)
  ...
  cmn::MediaType GetMediaType() const noexcept { return _media_type; }

  const std::shared_ptr<const ov::Data> GetData() const noexcept { return _data; }

  std::shared_ptr<ov::Data> &GetData() noexcept { return _data; }

  size_t GetDataLength() noexcept { return _data->GetLength(); }

  int64_t GetPts() const noexcept { return _pts; }

  void SetPts(int64_t pts) { _pts = pts; }

  int64_t GetDts() const noexcept { return _dts; }

  void SetDts(int64_t dts) { _dts = dts; }

  int64_t GetDuration() const noexcept { return _duration; }

  void SetDuration(int64_t duration) { _duration = duration; }

  int32_t GetTrackId() const noexcept { return _track_id; }

  void SetTrackId(int32_t track_id) { _track_id = track_id; }

  MediaPacketFlag GetFlag() const noexcept { return _flag; }
  void SetFlag(MediaPacketFlag flag) { _flag = flag; }

  cmn::BitstreamFormat GetBitstreamFormat() const noexcept { return _bitstream_format; }

  cmn::PacketType GetPacketType() const noexcept { return _packet_type; }

  void SetBitstreamFormat(cmn::BitstreamFormat format) { _bitstream_format = format; }

  void SetPacketType(cmn::PacketType type) { _packet_type = type; }

  void SetFragHeader(const FragmentationHeader *header) { _frag_hdr = *header; }

  FragmentationHeader *GetFragHeader() { return &_frag_hdr; }

  const FragmentationHeader *GetFragHeader() const { return &_frag_hdr; }

  std::shared_ptr<MediaPacket> ClonePacket() {
    auto packet = std::make_shared<MediaPacket>(GetMediaType(), GetTrackId(), GetData(), GetPts(), GetDts(),
                                                GetDuration(), GetFlag(), GetBitstreamFormat(), GetPacketType());
...
class MediaFrame {
 public:
  ...
  MediaFrame(cmn::MediaType media_type, int32_t track_id, const uint8_t *data, int32_t data_size, int64_t pts,
             int64_t duration, int32_t flags)
      : _media_type(media_type), _track_id(track_id), _pts(pts), _duration(duration), _flags(flags) {
  ...
  void SetMediaType(cmn::MediaType media_type) { _media_type = media_type; }

  cmn::MediaType GetMediaType() const { return _media_type; }

  void SetTrackId(int32_t track_id) { _track_id = track_id; }

  int32_t GetTrackId() const { return _track_id; }
  ...
  void SetStride(int32_t stride, int32_t plane = 0) { _stride[plane] = stride; }
  ...
  void SetFormat(int32_t format) { _format = format; }
  ...
  int32_t GetBytesPerSample() const { return _bytes_per_sample; }

  void SetBytesPerSample(int32_t bytes_per_sample) { _bytes_per_sample = bytes_per_sample; }

  int32_t GetNbSamples() const { return _nb_samples; }

  void SetNbSamples(int32_t nb_samples) { _nb_samples = nb_samples; }
  ...
  cmn::AudioChannel::Layout GetChannelLayout() const { return _channel_layout; }
  ...
  void SetSampleRate(int32_t sample_rate) { _sample_rate = sample_rate; }

  void SetFlags(int32_t flags) { _flags = flags; }

  int32_t GetFlags() const { return _flags; }
