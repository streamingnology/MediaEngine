...
bool AacConverter::GetExtraDataFromLatm(const cmn::PacketType type, const std::shared_ptr<ov::Data> &data,
...
std::shared_ptr<ov::Data> AacConverter::MakeAdtsHeader(uint8_t aac_profile, uint8_t aac_sample_rate,
                                                       uint8_t aac_channels, int16_t data_length) {
...
/*
  ADTS
  Unlike the ADIF header, ADTS (Audio Data Transport Stream) headers are present before each AAC raw_data_block or block
  of 2 to 4 raw_data_blocks. Until the MPEG revision from Dec 2002 for MPEG-4 AAC ADTS headers, this was basically the
  same as a MP3 header, except that the emphasis field was not present for MPEG-2 AAC, only for MPEG-4 AAC. Now the
  emphasis field (2 bits) has been abandoned completely, and thus MPEG-4 and MPEG-2 AAC ADTS headers are exactly the
  same except for the Object Type ID flag (MPEG-2 or MPEG-4). See also the Wiki page about MP4, because this is
  important when extracting MPEG-2 AAC files from a MP4 container with mp4creator or creating MP4 files from PsyTEL AAC
  encodings with this tool.
...
*/
bool AacConverter::ConvertLatmToAdts(const cmn::PacketType type, const std::shared_ptr<ov::Data> &data,
...
std::shared_ptr<const ov::Data> AacConverter::ConvertAdtsToLatm(const std::shared_ptr<const ov::Data> &data,
  ...
  while (remained > 0) {
    ...
    // K		1	copyrighted id bit, the next bit of a centrally registered copyright identifier, set to 0 when encoding,
    // ignore when decoding L		1	copyright id start, signals that this frame's copyright id bit is the first bit of the
    // copyright id, set to 0 when encoding, ignore when decoding M		13	frame length, this value must include 7 or 9
    // bytes of header length: FrameLength = (ProtectionAbsent == 1 ? 7 : 9) + size(AACFrame) O		11	Buffer fullness P
    // 2	Number of AAC frames (RDBs) in ADTS frame minus 1, for maximum compatibility always use 1 AAC frame per ADTS
    // frame Q		16	CRC if protection absent is 0
    ...
    // M - Frame length
    size_t frame_length =
        OV_GET_BITS(uint8_t, buffer[3], 0, 2) << 11 | (buffer[4] << 3) | OV_GET_BITS(uint8_t, buffer[5], 5, 3);
