 ...
 */
#include "snyaaclatmtoadts.h"
#include "aac_specific_config.h"
#include "core/bit_writer.h"

#define OV_LOG_TAG "AACLatmToAdts"
namespace sny {
bool SnyAACLatmToAdts::GetExtradata(const cmn::PacketType type, const std::shared_ptr<ov::Data> &data,
...
*/
bool SnyAACLatmToAdts::Convert(const cmn::PacketType type, const std::shared_ptr<ov::Data> &data,
                               const std::vector<uint8_t> &extradata, std::shared_ptr<ov::Data> &adts_data) {
  adts_data = std::make_shared<ov::Data>();
  adts_data->Clear();
...
std::shared_ptr<ov::Data> SnyAACLatmToAdts::MakeHeader(uint8_t aac_profile, uint8_t aac_sample_rate,
