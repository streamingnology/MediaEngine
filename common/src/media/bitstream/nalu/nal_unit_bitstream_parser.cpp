...
NalUnitBitstreamParser::NalUnitBitstreamParser(const uint8_t *bitstream, size_t length) : BitReader(nullptr, 0) {
  /*
  ...
  */
  for (size_t original_bitstream_offset = 0; original_bitstream_offset < length;) {
    if (original_bitstream_offset < length + 2 && bitstream[original_bitstream_offset] == 0x00 &&
        bitstream[original_bitstream_offset + 1] == 0x00 && bitstream[original_bitstream_offset + 2] == 0x03) {
...
bool NalUnitBitstreamParser::ReadU8(uint8_t &value) { return ReadBits(8, value); }

bool NalUnitBitstreamParser::ReadU16(uint16_t &value) { return ReadBits(16, value); }

bool NalUnitBitstreamParser::ReadU32(uint32_t &value) { return ReadBits(32, value); }
