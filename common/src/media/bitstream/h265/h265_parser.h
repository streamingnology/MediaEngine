// I developed the code of this file by referring to the source code of virinext's hevcsbrowser
// (https://github.com/virinext/hevcesbrowser). Thanks to virinext.
// - Getroot
...
class H265NalUnitHeader {
 public:
  H265NALUnitType GetNalUnitType() { return _type; }

  uint8_t GetLayerId() { return _layer_id; }

  uint8_t GetTemporalIdPlus1() { return _temporal_id_plus1; }
...
class H265SPS {
 public:
  ...
  uint8_t GetProfile() const { return _profile_tier_level._profile_idc; }
  uint8_t GetCodecLevel() const { return _profile_tier_level._level_idc; }
  float GetFps() const { return _vui_parameters._time_scale / _vui_parameters._num_units_in_tick; }
...
class H265Parser {
 ...
 private:
  ...
  static bool ProcessProfileTierLevel(uint32_t max_sub_layers_minus1, NalUnitBitstreamParser &parser,
                                      ProfileTierLevel &profile);
  static bool ProcessShortTermRefPicSet(uint32_t idx, uint32_t num_short_term_ref_pic_sets,
                                        const std::vector<ShortTermRefPicSet> &rpset_list,
                                        NalUnitBitstreamParser &parser, ShortTermRefPicSet &rpset);
  static bool ProcessVuiParameters(uint32_t max_sub_layers_minus1, NalUnitBitstreamParser &parser,
                                   VuiParameters &params);
  static bool ProcessHrdParameters(uint8_t common_inf_present_flag, uint32_t max_sub_layers_minus1,
                                   NalUnitBitstreamParser &parser, HrdParameters &params);
  static bool ProcessSubLayerHrdParameters(uint8_t sub_pic_hrd_params_present_flag, uint32_t cpb_cnt,
                                           NalUnitBitstreamParser &parser, SubLayerHrdParameters &params);
};
