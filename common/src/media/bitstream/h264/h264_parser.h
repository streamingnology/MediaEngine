...
class H264SPS {
 ...
 public:
  ...
  uint8_t GetCodecLevel() const { return _codec_level; }
  unsigned int GetFps() const { return _fps; }
  unsigned int GetId() const { return _id; }
  unsigned int GetMaxNrOfReferenceFrames() const { return _max_nr_of_reference_frames; }
  ...
  ov::String GetInfoString() {
    ...
    out_str.AppendFormat("\tAspectRatio(IDC - %d Extented - %d:%d)\n", _aspect_ratio_idc, _aspect_ratio._width,
                         _aspect_ratio._height);
...
class H264NalUnitHeader {
 public:
  H264NalUnitType GetNalUnitType() { return _type; }
