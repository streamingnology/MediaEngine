...
void H264BitstreamAnalyzer::ValidateNalUnit(const uint8_t *nal_unit_payload, size_t length,
                                            H264NalUnitType nal_unit_type, uint8_t nal_unit_header) {
    ...
    */
    ...
    if (parser.ReadUEV(first_mb_in_slice) && parser.ReadUEV(slice_type) && parser.ReadUEV(pps_id)) {
      ...
      if (expected_nal_ref_idc != -1 && nal_ref_idc != expected_nal_ref_idc) {
        logte("Invalid nal_ref_idc value %u for NAL unit type %u/slice type %u, expected value is %u", nal_ref_idc,
              nal_unit_type, slice_type, expected_nal_ref_idc);
        return;
      }
