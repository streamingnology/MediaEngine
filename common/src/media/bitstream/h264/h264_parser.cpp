...
bool H264Parser::ParseSPS(const uint8_t *nalu, size_t length, H264SPS &sps) {
  ...
  if (sps._profile == 44 || sps._profile == 83 || sps._profile == 86 || sps._profile == 100 || sps._profile == 110 ||
      sps._profile == 118 || sps._profile == 122 || sps._profile == 244) {
...

  {
    ...
    sps._height =
        ((2 - frame_mbs_only_flag) * (pic_height_in_map_units_minus1 + 1) * 16) - 2 * crop_top - 2 * crop_bottom;
  }
  ...
  if (vui_parameters_present_flag) {
    ...
    if (aspect_ratio_info_present_flag) {
      ...
      if (aspect_ratio_idc == 255) {
        if (parser.ReadU16(sps._aspect_ratio._width) == false || parser.ReadU16(sps._aspect_ratio._height) == false) {
          return false;
        }
      } else {
      ...
      if (video_signal_type_present_flag) {
        {
          ...
          if (!(parser.ReadBit(bit) && (video_format |= bit, video_format <<= 1, parser.ReadBit(bit)) &&
...

    {
      ...
      if (timing_info_present_flag) {
        ...
        if (!(parser.ReadUEV(num_units_in_tick) && parser.ReadUEV(time_scale) &&
              parser.ReadBit(fixed_frame_rate_flag))) {
          return false;
        }
        // loge("H264Sps", "fixed_frame_rate_flag(%d) time_scale(%d), num_units_in_tick(%d)",
        // (uint32_t)fixed_frame_rate_flag, (uint32_t)time_scale, (uint32_t)num_units_in_tick);
