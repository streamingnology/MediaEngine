...
bool AVCDecoderConfigurationRecord::Parse(const uint8_t *data, size_t data_length,
                                          AVCDecoderConfigurationRecord &record) {
  if (data_length < MIN_AVCDECODERCONFIGURATIONRECORD_SIZE) {
    logte("The data inputted is too small for parsing (%d must be bigger than %d)", data_length,
          MIN_AVCDECODERCONFIGURATIONRECORD_SIZE);
    return false;
  }
  ...
  if (record._profile_indication == 100 || record._profile_indication == 110 || record._profile_indication == 122 ||
      record._profile_indication == 144) {
...
uint8_t AVCDecoderConfigurationRecord::Version() { return _version; }

uint8_t AVCDecoderConfigurationRecord::ProfileIndication() { return _profile_indication; }

uint8_t AVCDecoderConfigurationRecord::Compatibility() { return _profile_compatibility; }

uint8_t AVCDecoderConfigurationRecord::LevelIndication() { return _level_indication; }

uint8_t AVCDecoderConfigurationRecord::LengthOfNALUnit() { return _lengthMinusOne; }

uint8_t AVCDecoderConfigurationRecord::NumOfSPS() { return _num_of_sps; }

uint8_t AVCDecoderConfigurationRecord::NumOfPPS() { return _num_of_pps; }

uint8_t AVCDecoderConfigurationRecord::NumofSPSExt() { return _num_of_sps_ext; }
...
uint8_t AVCDecoderConfigurationRecord::ChromaFormat() { return _chroma_format; }

uint8_t AVCDecoderConfigurationRecord::BitDepthLumaMinus8() { return _bit_depth_luma_minus8; }
...
void AVCDecoderConfigurationRecord::SetVersion(uint8_t version) { _version = version; }
...
void AVCDecoderConfigurationRecord::SetLengthOfNalUnit(uint8_t lengthMinusOne) { _lengthMinusOne = lengthMinusOne; }
