...
namespace sny {
SampleReader::SampleReader(AP4_Track& track, bool selected) : track_(track), selected_(selected), eos_(false) {
...
AP4_Result SampleReader::VideoConvertToSnyMediaSample(AP4_Sample& sample, AP4_DataBuffer& sample_data) {
...
AP4_Result SampleReader::AudioConvertToSnyMediaSample(AP4_Sample& sample, AP4_DataBuffer& sample_data) {
  AP4_SampleDescription* sample_description = track_.GetSampleDescription(sample.GetDescriptionIndex());
  if (!sample_description) {
    return AP4_ERROR_INVALID_PARAMETERS;
  }
...
AP4_Result SampleReader::OnSeek(AP4_UI64 timestamp_us, AP4_Ordinal& sample_index) {
  eos_ = false;
  while (!samples_.empty()) {
    auto sample = samples_.front();
    samples_.pop();
...
FragmentedSampleReader::FragmentedSampleReader(AP4_LinearReader& fragment_reader, AP4_Track& track, AP4_UI32 track_id,
                                               bool selected)
...
AP4_Result FragmentedSampleReader::ReadSample(AP4_Sample& sample, AP4_DataBuffer& sample_data) {
  AP4_Result result = fragment_reader_.ReadNextSample(track_.GetId(), sample, sample_data);
  return result;
}
