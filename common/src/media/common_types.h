...
enum class ProviderStreamDirection : int8_t { Pull, Push };
...
struct FragmentationHeader {
 public:
  ...
  bool operator==(const FragmentationHeader &other) const {
    return fragmentation_offset == other.fragmentation_offset && fragmentation_length == other.fragmentation_length &&
  ...
  bool operator!=(const FragmentationHeader &other) { return !(other == *this); }
  ...
  bool IsEmpty() const { return fragmentation_offset.empty() && fragmentation_length.empty(); }
