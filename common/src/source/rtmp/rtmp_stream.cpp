...
void RtmpStream::onDataReceived(const char *data_buffer, int data_size) {
  ...
  if (_remained_data->GetLength() > RTMP_MAX_PACKET_SIZE) {
    auto log = ov::String::FormatString(
        "The packet is ignored because the size is too large: [%s]), packet size: %zu, threshold: %d", _full_url.CStr(),
...
void RtmpStream::OnAmfConnect(const std::shared_ptr<const RtmpChunkHeader> &header, AmfDocument &document,
  ...
  // Parse the URL to obtain the domain name
  {
    ...
    if (_url != nullptr) {
      ...
      // So, if there is currently one VHost associated with the RTMP Provider, we need to modifiy it to work without
      // tcUrl.
...
bool RtmpStream::CheckSignedPolicy() { return true; }
void RtmpStream::OnAmfFCPublish(const std::shared_ptr<const RtmpChunkHeader> &header, AmfDocument &document,
                                double transaction_id) {
  if (_stream_name.IsEmpty() && document.GetProperty(3) != nullptr &&
    ...
    if (_url->Port() == 0) {
      ...
      if (conn_ != nullptr) {
        ...
        ::uv_tcp_getpeername(conn_->handle_.get(), (struct sockaddr *)&addr, &len);
        std::string addrStr;
        uint16_t port = uv::SocketAddr::GetIpAndPort(&addr, addrStr, uv::SocketAddr::IPV::Ipv4);
...
void RtmpStream::OnAmfPublish(const std::shared_ptr<const RtmpChunkHeader> &header, AmfDocument &document,
                              double transaction_id) {
...
bool RtmpStream::OnAmfMetaData(const std::shared_ptr<const RtmpChunkHeader> &header, AmfDocument &document,
...
bool RtmpStream::CheckReadyToPublish() {
  ...
  else if (_media_info->video_stream_coming == true &&
           _stream_message_cache_video_count > MAX_STREAM_MESSAGE_COUNT / 2) {
    return true;
  }
  // It makes this stream audio only
  else if (_media_info->audio_stream_coming == true &&
           _stream_message_cache_audio_count > MAX_STREAM_MESSAGE_COUNT / 2) {
    return true;
  }
...
bool RtmpStream::ReceiveVideoMessage(const std::shared_ptr<const RtmpMessage> &message) {
  ...
  if ((message->header->payload_size < RTMP_VIDEO_DATA_MIN_SIZE) ||
    ...
    logte("Size Fail - stream(%s/%s) size(%d)", _vhost_app_name.CStr(), _stream_name.CStr(),
  ...
  if (!IsPublished()) {
    ...
    if (CheckReadyToPublish()) {
      ...
      if (_stream_message_cache.size() > MAX_STREAM_MESSAGE_COUNT) {
        logtw("Rtmp input stream init meessage count over -  stream(%s/%s) size(%d:%d)", _vhost_app_name.CStr(),
              _stream_name.CStr(), _stream_message_cache.size(), MAX_STREAM_MESSAGE_COUNT);
      }
  ...
  if (_media_info->video_stream_coming) {
    ...
    if (!FlvVideoData::Parse(message->payload->GetDataAs<uint8_t>(), message->payload->GetLength(), flv_video)) {
    ...
    auto video_frame =
        std::make_shared<MediaPacket>(cmn::MediaType::Video, RTMP_VIDEO_TRACK_ID, data, pts, dts, 0, media_packet_flag,
    ...
    if (check_gap >= 60) {
      logtd(
          "Rtmp Provider Info - stream(%s/%s) key(%ums) timestamp(v:%ums/a:%ums/g:%dms) fps(v:%u/a:%u) "
          "gap(v:%ums/a:%ums)",
          _vhost_app_name.CStr(), _stream_name.CStr(), _key_frame_interval, _last_video_timestamp,
          _last_audio_timestamp, _last_video_timestamp - _last_audio_timestamp, _video_frame_count / check_gap,
          _audio_frame_count / check_gap, _last_video_timestamp - _previous_last_video_timestamp,
...
bool RtmpStream::ReceiveAudioMessage(const std::shared_ptr<const RtmpMessage> &message) {
  ...
  if (_media_info->audio_stream_coming) {
    ...
    if (!FlvAudioData::Parse(message->payload->GetDataAs<uint8_t>(), message->payload->GetLength(), flv_audio)) {
    ...
    auto audio_frame = std::make_shared<MediaPacket>(cmn::MediaType::Audio, RTMP_AUDIO_TRACK_ID, data, pts, dts, 0,
                                                     MediaPacketFlag::Key, cmn::BitstreamFormat::AAC_LATM, packet_type);
...
bool RtmpStream::SendMessagePacket(std::shared_ptr<RtmpMuxMessageHeader> &message_header,
...
bool RtmpStream::SendWindowAcknowledgementSize(uint32_t size) {
  ...
  auto message_header = std::make_shared<RtmpMuxMessageHeader>(
      RTMP_CHUNK_STREAM_ID_URGENT, 0, RTMP_MSGID_WINDOWACKNOWLEDGEMENT_SIZE, _rtmp_stream_id, body->size());
...
bool RtmpStream::SendSetPeerBandwidth(uint32_t bandwidth) {
  ...
  auto message_header = std::make_shared<RtmpMuxMessageHeader>(
      RTMP_CHUNK_STREAM_ID_URGENT, 0, RTMP_MSGID_SET_PEERBANDWIDTH, _rtmp_stream_id, body->size());
...
bool RtmpStream::SendAmfConnectResult(uint32_t chunk_stream_id, double transaction_id, double object_encoding) {
  auto message_header =
      std::make_shared<RtmpMuxMessageHeader>(chunk_stream_id, 0, RTMP_MSGID_AMF0_COMMAND_MESSAGE, _rtmp_stream_id, 0);
  AmfDocument document;
...
bool RtmpStream::SendAmfCreateStreamResult(uint32_t chunk_stream_id, double transaction_id) {
  auto message_header =
      std::make_shared<RtmpMuxMessageHeader>(chunk_stream_id, 0, RTMP_MSGID_AMF0_COMMAND_MESSAGE, 0, 0);
  AmfDocument document;
...
bool RtmpStream::SendAmfOnStatus(uint32_t chunk_stream_id, uint32_t stream_id, char *level, char *code,
                                 char *description, double client_id) {
  auto message_header =
      std::make_shared<RtmpMuxMessageHeader>(chunk_stream_id, 0, RTMP_MSGID_AMF0_COMMAND_MESSAGE, stream_id, 0);
  AmfDocument document;
  AmfObject *object = nullptr;
...
bool RtmpStream::ConvertToSnyMediaSample(std::shared_ptr<MediaTrack> &media_track,
  ...
  if (media_packet->GetMediaType() == cmn::MediaType::Video) {
    ...
    auto video_media_sample = std::make_shared<sny::SnyMediaSample>(
        ...
        media_packet->GetDuration(), media_track);
