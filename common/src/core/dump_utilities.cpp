...
String Dump(const void *data, size_t length, const char *title, off_t offset, size_t max_bytes,
            const char *line_prefix) noexcept {
  ...
  if (offset > 0) {
    ...
    dump.AppendFormat("%s========== %s 0x%08X + 0x%08X (%d/%" PRIi64 " bytes) ==========", line_prefix, title, data,
                      offset, dump_bytes, length);
  } else {
    // ========== xxxxx 0x12345678 (102400 / 1024000) ==========
    dump.AppendFormat("%s========== %s 0x%08X (%d/%" PRIi64 " bytes) ==========", line_prefix, title, data, dump_bytes,
                      length);
  }
...
std::shared_ptr<FILE> DumpToFile(const char *file_name, const void *data, size_t length, off_t offset,
...
std::shared_ptr<FILE> DumpToFile(const char *file_name, const std::shared_ptr<const Data> &data, off_t offset,
                                 bool append) noexcept {
  if (data == nullptr) {
    return nullptr;
  }
