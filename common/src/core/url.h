...
class Url {
 public:
  ...
  const ov::String &Source() const { return _source; }

  const ov::String &Scheme() const { return _scheme; }
  ...
  const ov::String &App() const { return _app; }

  const ov::String &Stream() const { return _stream; }

  const ov::String &File() const { return _file; }

  bool HasQueryString() const { return _has_query_string; }
