...
template <typename T, unsigned int B = sizeof(T), bool is_signed = std::numeric_limits<T>::is_signed>
class ByteReader;

template <typename T>
class ByteReader<T, 1, false> {
 public:
  static T ReadBigEndian(const uint8_t *data) { return data[0]; }

  static T ReadLittleEndian(const uint8_t *data) { return data[0]; }
};
...
class ByteWriter<T, 1, false> {
 public:
  static void WriteBigEndian(uint8_t *data, T val) { data[0] = val; }

  static void WriteLittleEndian(uint8_t *data, T val) { data[0] = val; }
};

template <typename T>
class ByteReader<T, 2, false> {
 public:
  static T ReadBigEndian(const uint8_t *data) { return (data[0] << 8) | data[1]; }

  static T ReadLittleEndian(const uint8_t *data) { return data[0] | (data[1] << 8); }
};
...
class ByteReader<T, 4, false> {
 ...
 private:
  inline static T Get(const uint8_t *data, unsigned int index) { return static_cast<T>(data[index]); }
};
...
class ByteReader<T, 8, false> {
 public:
  ...
  static T ReadLittleEndian(const uint8_t *data) {
    return Get(data, 0) | (Get(data, 1) << 8) | (Get(data, 2) << 16) | (Get(data, 3) << 24) | (Get(data, 4) << 32) |
           (Get(data, 5) << 40) | (Get(data, 6) << 48) | (Get(data, 7) << 56);
  }
