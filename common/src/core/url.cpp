...
std::shared_ptr<Url> Url::Parse(const ov::String &url) {
  ...
  if (!std::regex_search(url_string, matches,
                         std::regex(R"((.+?)://([^:/]+)(:([0-9]+))?(/([^\?]+)?)?(\?([^\?]+)?(.+)?)?)"))) {
    return nullptr;
  }
...
void Url::Print() const {
  logi("URL Parser", "%s %s %d %s %s %s %s", _scheme.CStr(), _host.CStr(), _port, _app.CStr(), _stream.CStr(),
       _file.CStr(), _query_string.CStr());
}

ov::String Url::ToUrlString(bool include_query_string) const {
  return ov::String::FormatString("%s://%s%s%s%s%s", _scheme.CStr(), _host.CStr(),
                                  (_port > 0) ? ov::String::FormatString(":%d", _port).CStr() : "", _path.CStr(),
                                  (!include_query_string || _query_string.IsEmpty()) ? "" : "?",
                                  include_query_string ? _query_string.CStr() : "");
}

ov::String Url::ToString() const {
  ov::String description;

  description.AppendFormat("%s://%s%s%s%s%s (app: %s, stream: %s, file: %s)", _scheme.CStr(), _host.CStr(),
                           (_port > 0) ? ov::String::FormatString(":%d", _port).CStr() : "", _path.CStr(),
                           _query_string.IsEmpty() ? "" : "?", _query_string.CStr(), _app.CStr(), _stream.CStr(),
                           _file.CStr());
