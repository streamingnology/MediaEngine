...
class Data {
 public:
  ...
  ///
  /// @remarks
  /// If reference_only is true, there is a problem if the data is changed or released because the memory is referenced
  /// without allocating memory separately. If reference_only is false, it will not be affected if the data changes
  /// because it allocates a new memory and copies it there.
  ...
  inline const uint8_t At(off_t index) const { return AtAs<uint8_t>(index); }
  ...
  // For debugging
  inline size_t GetAllocatedDataSize() const { return (_allocated_data != nullptr) ? _allocated_data->size() : 0ULL; }
  ...
  inline size_t GetCapacity() const noexcept { return (_allocated_data != nullptr) ? _allocated_data->capacity() : 0; }
  ...
  ///
  /// @param offset 원본 데이터에서 참조할 offset. 만약 offset이 음수라면, <GetLength() + offset> 부분부터 참조하는
  /// instance를 생성함
  ...
  bool IsEqual(const Data *data) const;
  bool IsEqual(const std::shared_ptr<Data> &data) const { return IsEqual(data->GetData(), data->GetLength()); }

  bool IsEmpty() const;

  String Dump(size_t max_bytes = 1024) const noexcept;
  String Dump(const char *title, const char *line_prefix) const noexcept;
  String Dump(const char *title, off_t offset = 0, size_t max_bytes = 1024,
              const char *line_prefix = nullptr) const noexcept;
