...
ByteStream::ByteStream(Data *data) : ByteStream(data, data, nullptr, 0L) {}

ByteStream::ByteStream(const std::shared_ptr<ov::Data> &data) : ByteStream(data.get(), data.get(), data, 0L) {}

ByteStream::ByteStream(const Data *data) : ByteStream(nullptr, data, nullptr, 0L) {}

ByteStream::ByteStream(const std::shared_ptr<const ov::Data> &data) : ByteStream(nullptr, data.get(), data, 0L) {}
...
size_t ByteStream::Remained() const noexcept { return Remained<uint8_t>(); }

bool ByteStream::IsRemained(size_t bytes) const noexcept { return Remained() >= bytes; }

bool ByteStream::IsEmpty() const noexcept { return (Remained() == 0); }
...
std::shared_ptr<const Data> ByteStream::GetDataPointer() const { return _data_pointer; }

std::shared_ptr<const Data> ByteStream::GetRemainData() const noexcept { return _read_only_data->Subdata(_offset); }

off_t ByteStream::GetOffset() const noexcept { return _offset; }
