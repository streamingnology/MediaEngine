...
class String {
 public:
  ...
  // 수정 가능한 버퍼 반환. 반드시 SetLength()를 호출하였거나, 길이를 확실하게 알고 있는 경우에만 이 버퍼를 받아서
  // 사용해야 함
...
struct CaseInsensitiveComparator {
  bool operator()(const String &s1, const String &s2) const { return s1.UpperCaseString() < s2.UpperCaseString(); }
};
}  // namespace ov
