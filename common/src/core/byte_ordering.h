...
#define OV_SELECT_BY_ENDIAN(call_when_little_endian, call_when_big_endian) call_when_little_endian
#else  // OV_IS_LITTLE_ENDIAN
#define OV_SELECT_BY_ENDIAN(call_when_little_endian, call_when_big_endian) call_when_big_endian
#endif  // OV_IS_LITTLE_ENDIAN

#define OV_DECLARE_ENDIAN_FUNCTION(type, name, func) \
  inline type name(type value) { return func(value); }

inline uint24_t be24toh(uint24_t value) noexcept { return OV_SELECT_BY_ENDIAN(be32toh(value << 8), value); }

inline uint24_t htobe24(uint24_t value) noexcept { return OV_SELECT_BY_ENDIAN(htobe32(value) >> 8, value); }

inline uint24_t le24toh(uint24_t value) noexcept { return OV_SELECT_BY_ENDIAN(value, le32toh(value << 8)); }

inline uint24_t htole24(uint24_t value) noexcept { return OV_SELECT_BY_ENDIAN(value, htole32(value) >> 8); }

// 기존에 endian.h 에 define 되어 있는 값들은 타입 강제성이 없어서 사용하기 불편한 부분이 있음
// 이를 해결하기 위해 재정의
namespace ov {
inline bool IsLittleEndian() noexcept { return OV_SELECT_BY_ENDIAN(false, true); }

inline bool IsBigEndian() noexcept { return OV_SELECT_BY_ENDIAN(false, true); }
