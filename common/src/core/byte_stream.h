...
#define OV_DECLARE_READ_FUNCTION(type, name, func) \
  inline type name() noexcept { return func(*Read<type>()); }

#define OV_DECLARE_WRITE_FUNCTION(type, name, func) \
  inline bool name(type value) noexcept { return WriteInternal<type>(func(value)); }
...
class ByteStream {
 public:
  ...
  /// 만약, _data가 is_reference 타입이라면 false가 반환될 수 있음.
  /// 데이터를 기록할 공간이 부족할 경우 _data에 Append() 됨.
  bool Write(const std::shared_ptr<const Data> &data) noexcept { return Write(data->GetData(), data->GetLength()); }
  ...
  /// @remarks
  /// 만약, _data가 is_reference 타입이라면 false가 반환될 수 있음.
  bool Append(const std::shared_ptr<const Data> &data) noexcept { return Append(data->GetData(), data->GetLength()); }
  ...
  /// 여러 곳에서 stream을 사용할 때, offset을 백업 & 복원하면서 사용해야 하는 이슈가 있는데, 이를 편하게 해주는
  /// 유틸리티 함수 (예: 임의의 위치로 이동한 뒤 다시 원래 자리로 오는 것)
  ///
  ...
  ///
  /// @return 성공적으로 offset이 복원되었는지 여부. 만약 offset history에 항목이 없거나 SetOffset()에 실패하면 false를
  /// 반환함.
  ///
  /// @see SetOffset()
