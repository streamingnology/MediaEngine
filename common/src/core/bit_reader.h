...
class BitReader {
 public:
  BitReader(const uint8_t* buffer, size_t capacity) : _buffer(buffer), _position(buffer), _capacity(capacity) {}
  ...
  bool SkipAll() { return SkipBytes(BytesReamined()); }
  ...
  void StartSection() { _lap_position = _position; }
  ...
  const uint8_t* CurrentPosition() { return _position; }

  size_t BytesReamined() const { return _capacity - BytesConsumed(); }

  size_t BytesConsumed() const { return _position - _buffer; }

  size_t BitsConsumed() const { return (_position - _buffer) * 8 + _bit_offset; }
