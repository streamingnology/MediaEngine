...
Data::Data(size_t capacity) { Reserve(capacity); }
...
std::shared_ptr<Data> Data::Clone() const { return std::const_pointer_cast<Data>(Subdata(0L)); }
...
std::shared_ptr<const Data> Data::Subdata(off_t offset, size_t length) const { return SubdataInternal(offset, length); }
...
bool Data::operator==(const Data &data) const { return IsEqual(data); }
...
bool Data::IsEqual(const Data &data) const { return IsEqual(data.GetData(), data.GetLength()); }
...
bool Data::IsEmpty() const { return (GetLength() == 0); }
...
bool Data::Append(const void *data, size_t length) { return Insert(data, GetLength(), length); }

bool Data::Append(const Data *data) { return (data != nullptr) ? Append(data->GetData(), data->GetLength()) : false; }

bool Data::Append(const std::shared_ptr<Data> &data) { return Append(data.get()); }

bool Data::Append(const std::shared_ptr<const Data> &data) { return Append(data.get()); }
...
String Data::Dump(size_t max_bytes) const noexcept { return Dump(nullptr, 0L, max_bytes, nullptr); }
...
String Data::ToString() const { return String(GetDataAs<const char>(), GetLength()); }
