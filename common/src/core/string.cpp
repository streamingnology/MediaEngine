...
namespace ov {
String::String(const char *string) { Append(string); }

String::String(const char *string, size_t length) { Append(string, length); }

String::String(const String &string) : String(string._buffer, string.GetLength()) {}
...
String::operator const char *() const noexcept { return CStr(); }
...
std::vector<String> String::Split(const char *string, const char *separator, size_t max_count) const {
  ...
  while (token_count < max_count) {
    ...
    auto length = ((last == nullptr) || (token_count == (max_count - 1))) ? (::strlen(string) * sizeof(char))
                                                                          : ((last - string) * sizeof(char));
...
bool String::HasPrefix(String prefix) const { return (Left(prefix.GetLength()) == prefix); }

bool String::HasSuffix(String suffix) const { return (Right(suffix.GetLength()) == suffix); }
...
char String::operator[](off_t index) const { return Get(index); }

bool String::operator!=(const char *buffer) const { return ((operator==(buffer)) == true) ? false : true; }
...
size_t String::GetCapacity() const noexcept { return _capacity; }

bool String::SetCapacity(size_t length) noexcept { return Alloc(length, true); }

size_t String::GetLength() const noexcept { return _length; }
...
bool String::Clear() { return Release(); }

bool String::IsEmpty() const noexcept { return (_length == 0L); }
