...
class Queue {
 public:
  ...
  Queue(const char *alias, size_t threshold = 0, int log_interval_in_msec = 5000)
      : _threshold(threshold), _log_interval(log_interval_in_msec) {
    SetAlias(alias);

    _last_log_time.Start();

    auto shared_lock = std::shared_lock(_name_mutex);
    logd("ov.Queue", "[%p] %s is created with threshold: %zu, interval: %d", this, _queue_name.CStr(), threshold,
         log_interval_in_msec);
  }
  ...
  std::optional<T> Dequeue(int timeout = Infinite) {
    ...
    if (_stop == false) {
      std::chrono::system_clock::time_point expire =
          (timeout == Infinite) ? std::chrono::system_clock::time_point::max()
                                : std::chrono::system_clock::now() + std::chrono::milliseconds(timeout);

      auto result =
          _condition.wait_until(unique_lock, expire, [this]() -> bool { return ((_queue.empty() == false) || _stop); });
  ...
  inline void CheckThreshold() {
    if ((_threshold > 0) && (_queue.size() >= _threshold)) {
      if (_last_log_time.IsElapsed(_log_interval) && _last_log_time.Update()) {
        auto shared_lock = std::shared_lock(_name_mutex);
        logw("ov.Queue", "[%p] %s size has exceeded the threshold: queue: %zu, threshold: %zu", this,
             _queue_name.CStr(), _queue.size(), _threshold);
      }
    }
  }
